...
try:
    ...
    typer.secho("  uv tool upgrade --with openapi-burrito[preview] openapi-burrito", fg=typer.colors.CYAN)
...
def run_preview(input_source: str):
    ...
    try:
        if input_source.startswith(("http://", "https://")):
            ...
            typer.secho(f"\n❌ Unsupported file format for {input_source}. Use .json or .yaml", fg=typer.colors.RED,
                        bold=True)
            raise typer.Exit(code=1)
    except Exception as e:
        typer.secho(f"\n❌ Failed to load spec from {input_source}: {e}", fg=typer.colors.RED, bold=True)
        raise typer.Exit(code=1)

    # Disable the default openapi, docs, and redoc so they don't conflict with ours
    app_ui = FastAPI(
        title="SDK Spec Preview",
        openapi_url=None,
        docs_url=None,
        redoc_url=None
    )
    ...
    typer.echo(f"  - Swagger UI: ", nl=False)
    typer.secho(f"http://127.0.0.1:8000/docs", fg=typer.colors.CYAN, underline=True)
    typer.echo(f"  - Redoc:      ", nl=False)
    typer.secho(f"http://127.0.0.1:8000/redoc", fg=typer.colors.CYAN, underline=True)

    uvicorn.run(app_ui, host="127.0.0.1", port=8000, log_level="error")
